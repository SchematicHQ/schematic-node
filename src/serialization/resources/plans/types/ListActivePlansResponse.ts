/**
 * This file was auto-generated by Fern from our API Definition.
 */

import * as serializers from "../../../index";
import * as Schematic from "../../../../api/index";
import * as core from "../../../../core";
import { PlanDetailResponseData } from "../../../types/PlanDetailResponseData";

export const ListActivePlansResponse: core.serialization.ObjectSchema<
    serializers.ListActivePlansResponse.Raw,
    Schematic.ListActivePlansResponse
> = core.serialization.object({
    data: core.serialization.list(PlanDetailResponseData),
    params: core.serialization.record(core.serialization.string(), core.serialization.unknown()),
});

export declare namespace ListActivePlansResponse {
    interface Raw {
        data: PlanDetailResponseData.Raw[];
        params: Record<string, unknown>;
    }
}
