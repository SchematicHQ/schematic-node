// This file was auto-generated by Fern from our API Definition.

import type * as Schematic from "../../../../api/index";
import * as core from "../../../../core";
import type * as serializers from "../../../index";
import { IntegrationInstallResponseData } from "../../../types/IntegrationInstallResponseData";

export const InstallIntegrationResponse: core.serialization.ObjectSchema<
    serializers.InstallIntegrationResponse.Raw,
    Schematic.InstallIntegrationResponse
> = core.serialization.object({
    data: IntegrationInstallResponseData,
    params: core.serialization.record(core.serialization.string(), core.serialization.unknown()),
});

export declare namespace InstallIntegrationResponse {
    export interface Raw {
        data: IntegrationInstallResponseData.Raw;
        params: Record<string, unknown>;
    }
}
