/**
 * This file was auto-generated by Fern from our API Definition.
 */

import * as serializers from "../../../index";
import * as Schematic from "../../../../api/index";
import * as core from "../../../../core";
import { BillingSubscriptionResponseData } from "../../../types/BillingSubscriptionResponseData";

export const ChangeSubscriptionWebResponse: core.serialization.ObjectSchema<
    serializers.ChangeSubscriptionWebResponse.Raw,
    Schematic.ChangeSubscriptionWebResponse
> = core.serialization.object({
    data: BillingSubscriptionResponseData,
    params: core.serialization.record(core.serialization.string(), core.serialization.unknown()),
});

export declare namespace ChangeSubscriptionWebResponse {
    interface Raw {
        data: BillingSubscriptionResponseData.Raw;
        params: Record<string, unknown>;
    }
}
