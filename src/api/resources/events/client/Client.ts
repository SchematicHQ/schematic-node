...
export class Events {
   ...
   */
  public async getEvent(eventId: string, requestOptions?: Events.RequestOptions): Promise<Schematic.GetEventResponse> {
    const _response = await (this._options.fetcher ?? core.fetcher)({
      url: urlJoin(
        (await core.Supplier.get(this._options.environment)) ?? environments.SchematicEnvironment.Default,
  ...
  public async listMetricCounts(
    ...
    const { startTime, endTime, eventSubtype, eventSubtypes, companyId, companyIds, userId, limit, offset, grouping } =
      request;
    const _queryParams: Record<string, string | string[] | object | object[]> = {};
    if (startTime != null) {
